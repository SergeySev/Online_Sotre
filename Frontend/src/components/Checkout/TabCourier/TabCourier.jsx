...
export function TabCourier() {
	...
	const [values, setValues] = useState({
		...
		postCode: post_code ? post_code : isAuth.postCode,
		street: street ? street : isAuth.street,
		houseNumber: house ? house : isAuth.houseNumber,
		apartmentNumber: app ? app : isAuth.apartmentNumber,
	...
	function printDayFromMillisec(millisec) {
		...
		return `${date_local.getDate()}.${
			date_local.getMonth() + 1
		}.${date_local.getFullYear()}`;
	}

	const setCalendarValue = (date) => {
		const day_format = `${date.getDate()}.${
			date.getMonth() + 1
		}.${date.getFullYear()}`;
		...
		setValues({
			...values,
			date: day_format,
			shipping: setDeliveryShipping(date.getTime()),
		});
	};
